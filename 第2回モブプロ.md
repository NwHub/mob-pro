...
- サーバー起動（確認）：`npm run serve`
- フォーマット実行：`npm run format`
- テスト実行：`npm run test:unit`(スナップショットの更新：`npm run test:unit -- -u`)
